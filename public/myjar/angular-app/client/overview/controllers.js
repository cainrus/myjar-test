...
angular.module('client.overview.controllers', []).
    ...
    ['$scope', '$rootScope', 'loanFactory', 'ModalService', function ($scope, $rootScope, loanFactory, ModalService) {
        ...
        $scope.getInterest = function() {
            return getProductValue('interest');
        };

        var getProductValue = function(key, tab) {
            tab = tab || ($scope.tabs && $scope.tabs[selectedTabIndex]) || {};
            return tab[key] || '';
        };

        $scope.isTunableDay = function() {
            return Boolean(Number(getProductValue('tunable_time')));
        };

        $scope.getTabColorStyle = function(tab) {
            var color = getProductValue('color_hex', tab);
